---
title: "Run models in R-STAN with NO2 swapped with NOX in models"
...
for(j in c(1:length(datasets))){
  ...
  if((file.exists(paste0(output.no2.folder,model_current,'.rds')))){
  ...
  if(!(file.exists(paste0(output.no2.folder,model_current,'.rds')))){
    ...
    if(i %in% c(101,106:115)){ 
      # traffic pollutants
      x <- model.matrix(ck ~ NO2 + CO + EC, data=get(datasets[j]))
      ...
      if(i %in% c(101,106:113)){
        assign(model_current, 
               stan(file = paste0('code/models/stan/stan_no2/fml',i,'.stan'), data = data))
      }
      # for models using greater numbers of iterations and warm-up
      if(i %in% c(114:115)){
        assign(model_current, 
               stan(file = paste0('code/models/stan/stan_no2/fml',i,'.stan'), data = data, iter=15000, warmup=5000))
      }
    } 
    ...
    if(i %in% c(102)){ 
      ...
      data <- 
        list(N = nrow(x), # number of total cases and controls
             ...
             x3 = x3, # covariates
             ck = get(datasets[j])$ck) # case or control marker
      assign(model_current, stan(file = paste0('code/models/stan/stan_no2/fml',i,'.stan'), data = data))
    }  
    
    # NO2 single traffic pollutant model
    if(i %in% c(103)){ 
      # traffic pollutants
      x <- model.matrix(ck ~ NO2, data=get(datasets[j]))
    ...
    # if(i %in% c(116)){ 
    #   # traffic pollutants
    #   x <- model.matrix(ck ~ NO2 + CO + EC, data=get(datasets[j]) %>% tidyr::drop_na(parish_ses))
    #   x <- x[,c(2:ncol(x))] # get rid of intercept column
    #   # non-traffic pollutants
    ...
    # 
    #     assign(model_current, stan(file = paste0('code/models/stan/stan_no2/fml',i,'.stan'), data = data, iter=50))
    # } 
       
  # save model  
  saveRDS(get(model_current),paste0(output.no2.folder,model_current,'.rds'))
